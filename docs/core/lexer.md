...
- ASCII space (`U+0020`)Tab (`U+0009`)Line feed (`\n`, `U+000A`)Carriage return (`\r`, `U+000D`)
...
- The `)` and all characters until `\n`, `\r`, `\r\n`, or EOF are ignoredComments do not emit tokensInside a string literal `( … )`, the `)` closes the string, not a commentA `)` in the middle of a PATH is part of the path, not a comment starter
...
- Any character except `)` and backslash is literal`)` closes the stringBackslash (`\`) starts a Unicode escape
...
- Hex digits are decoded using `int(hex, 16)`The result is converted to Unicode using `chr(codepoint)`Empty escapes `\\` are illegal
...
- `>{...}` is valid: EXEC followed by LBRACE`!{...}` is **illegal**: cannot store directly into a block
...
- `>print` → Two tokens: EXEC(`>`) + PATH(`print`)`>_.action` → Two tokens: EXEC(`>`) + PATH(`_.action`)`>{ ... }` → EXEC(`>`) + LBRACE + ... + RBRACE

### Valid Targets:

- PATH: `>foo` → EXEC, PATH(`foo`)LBRACE: `>{ ... }` → EXEC, LBRACE, ...
...
- `!counter` → Two tokens: STORE(`!`) + PATH(`counter`)`!_.temp` → Two tokens: STORE(`!`) + PATH(`_.temp`)
...
- ****Modifier Form (produces EXEC or STORE token):****: **PATH Form (produces PATH token):**

- Followed immediately by a valid target (PATH, LBRACE for `>`)
- No whitespace between modifier and target

- Followed by whitespace, EOF, or `}`Appears mid-token: `a>b` → PATH(`a>b`), `x!y` → PATH(`x!y`)
...
- Leading sign (`+` or `-`) is optionalThe sign **must** be followed by a digit to form an integer
...
- `_` alone is a valid PATH token representing the register root`_.x`, `_.x.y`, etc. are valid PATH tokens for register paths`_tmp`, `_x`, `_self` (without dot after `_`) are lexically valid PATH tokens, but are **not** standard register syntax

The distinction between `_x` and `_.x` is **semantic**, not lexical:

- The lexer tokenizes both as PATH tokensThe **parser** validates register syntax: only `_` (register root) and paths starting with `_.` (e.g., `_.x`) are validPaths like `_x` (without dot) cause a `ParseError` with a helpful message suggesting the correct `_.x` form
...
| String`(` ... `)`STRINGUnicode escapes via `\HEX\`             |                 |                           |                          |
| Block open                                                     | `{`             | LBRACE                    | Structural delimiter     |
| Block close                                                    | `}`             | RBRACE                    | Structural delimiter     |
| Execute`>` + targetEXECPrefix modifier; `>print` is two tokens |                 |                           |                          |
| Store`!` + targetSTOREPrefix modifier; `!x` is two tokens      |                 |                           |                          |
...
Note: 
**>print**
 is two tokens—EXEC prefix modifier followed by PATH. The lexer does not interpret this as "calling a built-in". It simply recognizes the prefix modifier 
`>`
 and the path name 
`print`
. The execution semantics (reading and executing the Block at Store path "print") are handled by the parser/runtime.
(hello) !message
```
...
### The lexer does not enforce register syntax rules—it only produces PATH tokens. The parser or runtime must validate that register paths follow the  `_.path`  convention. 2.12.4 Comments and Strings
...
## Note: The first  `)`  inside the string does not start a comment; it closes the string. 2.13 Lexer Summary
...
2. String decoding happens in lexer
3. Unicode escapes (
4. `\HEX\`
5. ) are decoded during lexing; the parser receives the final decoded string
6. Register syntax is not enforced
7. The lexer accepts both 
8. `_x`
9.  and 
10. `_.x`
11.  as PATH tokens; the parser enforces the 
12. `_.x`
13.  convention
14. Modifier detection is context-sensitive
15. `>foo`
16.  produces EXEC + PATH, while 
17. `> foo`
18.  produces PATH + PATH
19. Error reporting includes location
20. `LexError`
21.  exceptions include line/col information for precise error messages

---

# 3. Parser and AST Construction

1. The SOMA parser (defined in 
2. `soma/parser.py`
3. ) consumes the token stream from the lexer and builds an Abstract Syntax Tree (AST). The parser is a recursive descent parser that:
4. Validates syntax and structure
5. Builds typed AST nodes
6. Splits paths into components
7. Validates semantic rules (like register path syntax)
8. Eagerly parses block contents
...
7. Calls 
8. parser.parse()
9. Converts AST nodes to dictionary representation for external use
...
- **Lookahead**: _peek()Token consumption_advance()Recursive parsingEach AST node type has its own parsing method
...
5.  to get 
6. `["a", "b", "c"]`
7. Parser validates register syntax if first component is 
8. `"_"`
...
The _parse_statement() method dispatches to appropriate parsers based on the current token type.
